...
[Serializable]
internal class TestShadowRegionProvider : TestShadowProviderBase
{
    private ShadowShape2D m_PersistantShadowShape;

    public override bool IsRequiredComponentData(Component sourceComponent) => sourceComponent is TestShadowRegion;
    ...
    public override void OnBeforeRender(Component sourceComponent, Bounds worldCullingBounds, ShadowShape2D persistantShadowShape)
    {
        ...
        if (testShadowRegion != null)
        {
            // Does it overlap the culling bounds?
            if (testShadowRegion.Overlap(worldCullingBounds))
            {
                // Yes, so fetch shapes that overlap the region.
                using var regionOverlaps = testShadowRegion.GetOverlaps();
