...
public class PhysicsDestructorFragmentMaskGeometry : MonoBehaviour
{
    ...
    private void CreateFragmentPoints()
    {
        ...
        for (var i = 0; i < FragmentPointCount; ++i)
            m_FragmentedPoints[i] = FragmentTransform.position + PhysicsRotate.FromRadians(m_Random.NextFloat(0f, PhysicsMath.TAU)).direction * m_Random.NextFloat(0f, FragmentMaskRadius);
    }
