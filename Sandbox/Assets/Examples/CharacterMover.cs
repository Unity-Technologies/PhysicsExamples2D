...
public sealed class CharacterMover : SandboxExampleBehaviour
{
    ...
    protected override Vector2 CameraPosition => new(20f, 5f);
    ...
    protected override void OnExampleEnable()
    {
        ...
        // Register the character movement.
    ...
    private void Update()
    {
        ...
        world.DrawLine(m_Transform.position, m_Transform.position + m_Velocity.normalized * VelocityDrawScale, ShapeColor);
    ...
    {
        // Jump Speed.
        AddSlider("Jump Speed", m_JumpSpeed, 0f, 40f, v => { m_JumpSpeed = v; });

        // Min Speed.
        AddSlider("Min Speed", m_MinSpeed, 0f, 1f, v => { m_MinSpeed = v; });

        // Max Speed.
        AddSlider("Max Speed", m_MaxSpeed, 0f, 20f, v => { m_MaxSpeed = v; });

        // Stop Speed.
        AddSlider("Stop Speed", m_StopSpeed, 0f, 10f, v => { m_StopSpeed = v; });

        // Accelerate.
        AddSlider("Accelerate", m_Accelerate, 0f, 100f, v => { m_Accelerate = v; });

        // Air Steer.
        AddSlider("Air Steer", m_AirSteer, 0f, 1f, v => { m_AirSteer = v; });

        // Friction.
        AddSlider("Friction", m_Friction, 0f, 10f, v => { m_Friction = v; });

        // Gravity.
        AddSlider("Gravity", m_Gravity, 0f, 100f, v => { m_Gravity = v; });

        // Pogo Scale.
        AddSlider("Pogo Scale", m_PogoScale, 1f, 10f, v => { m_PogoScale = v; });

        // Pogo Frequency.
        AddSlider("Pogo Frequency", m_PogoFrequency, 0f, 30f, v => { m_PogoFrequency = v; });

        // Pogo Damping.
        AddSlider("Pogo Damping", m_PogoDamping, 0f, 30f, v => { m_PogoDamping = v; });

        // Pogo Type.
        AddEnum("Pogo Type", m_PogoType, v => { m_PogoType = v; });
    }
    ...
    {
        // Finish if the world is paused.
        if (SandboxManager.WorldPaused || !world.isDefaultWorld)
            return;
        ...
        CameraManipulator.CameraPosition = new Vector2(m_Transform.position.x, CameraManipulator.CameraPosition.y);
    }
