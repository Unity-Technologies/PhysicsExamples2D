...
> note below and the §7 template. Never use the legacy `Physics2D` component system
...
> **`[ExampleScene]`**, with the repeated menu chrome factored into shared infrastructure. The
> `Example.cs` starter file uses this pattern. **Author all new and modified examples with the
> new pattern below.**
...
- [ ] Uses the cross-version `#if UNITY_6000_5_OR_NEWER` using-guard for the physics namespace.
...
- [ ] If asset dependencies are needed: companion `<Name>Data : ExampleSceneData` class defined,
      `<Name>Data.asset` created in `Assets/ExampleAssets/`, assets assigned in Inspector.
