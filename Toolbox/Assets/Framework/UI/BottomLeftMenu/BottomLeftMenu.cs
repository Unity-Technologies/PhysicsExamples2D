...
// Bottom-left panel hosting two mutually-exclusive sections — the "Toolbox" world/draw controls
// (queried + wired by ToolboxManager) and the "Shortcuts" global-control buttons. Two footer
...
// ToolboxManager wires the actual control behaviour.
public class BottomLeftMenu : MonoBehaviour, IFoldable
{
    ...
    private enum AccordionState { None, Toolbox, Shortcuts }

    // When true the panel starts with both sections collapsed.
    private const bool k_StartFolded = true;

    // The global-control buttons (wired by ToolboxManager).
    ...
    private VisualElement m_ToolboxDetails;
    private VisualElement m_ShortcutsDetails;
    private Button m_ToolboxFooter;
    ...
    private void SetState(AccordionState state)
    {
        ...
        m_ToolboxDetails.style.display = state == AccordionState.Toolbox ? DisplayStyle.Flex : DisplayStyle.None;
        m_ShortcutsDetails.style.display = state == AccordionState.Shortcuts ? DisplayStyle.Flex : DisplayStyle.None;

        m_ToolboxFooter.text = FooterText(state == AccordionState.Toolbox, "Toolbox");
    ...
    {
        var caret = expanded ? "▼" : "▲";
        return $"{ToolboxUtility.HighlightColor}{caret}{ToolboxUtility.EndHighlightColor}<size=50%> </size>{label}";
    }

    // Footer clicks: toggle this section, collapsing the other (accordion — never both open).
    private void ToggleToolbox() => SetState(m_State == AccordionState.Toolbox ? AccordionState.None : AccordionState.Toolbox);
    ...
    private void OnEnable()
    {
        ...
        m_ToolboxDetails = root.Q<VisualElement>("toolbox-details");
        m_ShortcutsDetails = root.Q<VisualElement>("shortcuts-details");

        // Footer toggles.
        m_ToolboxFooter = root.Q<Button>("toolbox-footer");
        m_ShortcutsFooter = root.Q<Button>("shortcuts-footer");
        m_ToolboxFooter.clicked += ToggleToolbox;
        m_ShortcutsFooter.clicked += ToggleShortcuts;

        // Global-control buttons (wired by ToolboxManager).
