...
/// <summary>
/// Makes the Toolbox the scene a new user starts from. Two parts:
///   1. The Play-mode start scene is (re)assigned the instant Play is pressed, so hitting Play
///      always boots the Toolbox regardless of which scene is open.
///   2. On the first editor load of a session (e.g. a fresh checkout that opened an empty Untitled
///      scene), the Toolbox scene is opened automatically so the user just sees it and can hit Play.
/// </summary>
[InitializeOnLoad]
public static class StartupScene
{
    private const string StartupScenePath = "Assets/Toolbox.unity";
    ...
    static StartupScene()
    {
        ...
        // original cause of "hitting Play doesn't start the Toolbox" on a new clone).
    ...
    {
        // Keep the Play-mode start scene pointing at the Toolbox (so it also shows correctly in
