...
public class PhysicsShapeContactCallback : MonoBehaviour, PhysicsCallbacks.IContactCallback
{
    ...
    private void OnEnable()
    {
        ...
        var body1 = m_PhysicsWorld.CreateBody(new PhysicsBodyDefinition { type = PhysicsBody.BodyType.Dynamic, position = new Vector2(-0.25f, 2f) });
        ...
        // NOTE: Here we contact the ground so if we were to enable contact events for it, we'd still receive callbacks when hitting the ground, however shape/shape contact would not.
        ...
        // NOTE: "AutoContactCallbacks" is normally off by default, but we have changed the "Physics LowLevel Settings > Physics World Definition" to default this to being enabled.
    ...
    // Called when a pair of shapes begin to contact.
