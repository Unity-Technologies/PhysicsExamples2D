...
{
    [EditorTool("Edit Test Outline Shape", typeof(TestOutlineShape))]
    internal sealed partial class TestOutlineShapeTool : EditorTool, IGeometryToolSettings
    {
        public override void OnActivated()
        {
            ...
            m_GeometryTools = new List<TestOutlineShapeGeometryEditorTool>(capacity: 1);
            foreach (var toolTarget in targets)
            {
                if (toolTarget is TestOutlineShape sceneOutlineShape)
                {
                    if (sceneOutlineShape.isActiveAndEnabled)
                        m_GeometryTools.Add(new TestOutlineGeometryEditorTool(sceneOutlineShape, this));
                }
            }
        }
        ...
        private List<TestOutlineShapeGeometryEditorTool> m_GeometryTools;
        private TestGeometryToolOverlay m_Overlay;

        /// <summary>
        /// Test Outline Geometry Tool.
        /// </summary>
        private sealed class TestOutlineGeometryEditorTool : TestOutlineShapeGeometryEditorTool
        {
            public TestOutlineGeometryEditorTool(TestOutlineShape testOutlineShape, IGeometryToolSettings geometryToolSettings) : base(testOutlineShape, geometryToolSettings)
            {
            }
                ...
                for (int i = 0, j = pointCount - 1; i < pointCount; j = i++)
                {
...

                    {
                        ...
                        using (new Handles.DrawingScope(Matrix4x4.TRS(point, Quaternion.identity, Vector3.one)))
                        {
                            ...
                            {
                                Undo.RecordObject(Target, "Change Outline Point");
                            ...
                            {
                                Undo.RecordObject(Target, "Delete Outline Point");
                    ...
                    if (addMode)
                    {
                        ...
                        using (new Handles.DrawingScope(Matrix4x4.TRS(point, Quaternion.identity, Vector3.one)))
                        {
                            ...
                            {
                                Undo.RecordObject(Target, "Add TestOutlineGeometry Point");
