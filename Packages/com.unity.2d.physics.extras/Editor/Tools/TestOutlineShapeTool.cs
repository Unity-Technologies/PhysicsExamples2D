...
namespace Unity.U2D.Physics.Editor.Extras
{
    ...
    internal sealed partial class TestOutlineShapeTool : EditorTool, IGeometryToolSettings
    {
        public override void OnActivated()
        {
            ...
            {
                if (toolTarget is TestOutlineShape testOutlineShape)
                {
                    if (testOutlineShape.isActiveAndEnabled)
                        m_GeometryTools.Add(new TestOutlineGeometryEditorTool(testOutlineShape, this));
                }
            }
        }
